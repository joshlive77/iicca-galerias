<div class="iicca-galerias eltdf-tabs eltdf-tabs-standard ui-tabs">

    <ul class="eltdf-tabs-nav ui-tabs-nav ">
        <li class="ui-state-default ui-tabs-active ui-state-active">
            <a href="#tab-bvxcb-114" class="ui-tabs-anchor" id="ui-id-1"> tag1 </a> 
        </li>
        <li class="ui-state-default ">
            <a href="#tab-bvxcb-188" class="ui-tabs-anchor" id="ui-id-2"> tag2 </a> 
        </li>
    </ul>

    <div class="contenedor-galerias eltdf-tab-container ui-tabs-panel ui-widget-content ui-corner-bottom eltdf-visible" id="tab-bvxcb-114" style="">
        <!-- <div class="wpb_text_column wpb_content_element "> -->
            <!-- <div class="wpb_wrapper"> -->
                <div class="iicca-lista-galerias">
                    <div class="galeria" style="display: block;">
                        <div class="contenedor">
                            <div class="imagen">
                                <img src="https://iicca.edu.bo/wp-content/uploads/2019/09/N66.jpg">
                            </div>
                            <div class="contenido">
                                <div class="titulo">
                                    <a href="https://iicca.edu.bo/galeria-prueba/">
                                        <h3>Lorem, ipsum dolor.</h3>
                                    </a>
                                </div>
                                <div class="etiquetas">
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                </div>
                                <div class="descripcion">
                                    <P>Lorem ipsumplicabo lerat libero.</P>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="galeria" style="display: block;">
                        <div class="contenedor">
                            <div class="imagen">
                                <img src="https://iicca.edu.bo/wp-content/uploads/2019/09/comex0.jpg">
                            </div>
                            <div class="contenido">
                                <div class="titulo">
                                    <a href="https://iicca.edu.bo/galeria-prueba/">
                                        <h3>Lorem, ipsum dolor.</h3>
                                    </a>
                                </div>
                                <div class="etiquetas">
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                </div>
                                <div class="descripcion">
                                    <P>Lorem ipsum dolor sit amet consectetur adipisicing elit. Repudiandae neque eius incidunt assumenda nesciunt eum explicabo laudantium, soluta quaerat libero.</P>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            <!-- </div> -->
        <!-- </div> -->
    </div>

    <div class="contenedor-galerias eltdf-tab-container ui-tabs-panel ui-widget-content ui-corner-bottom" id="tab-bvxcb-188" style="display: none;">
        <!-- <div class="wpb_text_column wpb_content_element "> -->
            <!-- <div class="wpb_wrapper"> -->
                <div class="iicca-lista-galerias">
                    <div class="galeria" style="display: block;">
                        <div class="contenedor">
                            <div class="imagen">
                                <img src="https://iicca.edu.bo/wp-content/uploads/2017/10/N2.jpg">
                            </div>
                            <div class="contenido">
                                <div class="titulo">
                                    <a href="https://iicca.edu.bo/galeria-prueba/">
                                        <h3>Lorem, ipsum dolor.</h3>
                                    </a>
                                </div>
                                <div class="etiquetas">
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="galeria" style="display: block;">
                        <div class="contenedor">
                            <div class="imagen">
                                <img src="https://iicca.edu.bo/wp-content/uploads/2019/09/N66.jpg">
                            </div>
                            <div class="contenido">
                                <div class="titulo">
                                    <a href="https://iicca.edu.bo/galeria-prueba/">
                                        <h3>Lorem, ipsum dolor.</h3>
                                    </a>
                                </div>
                                <div class="etiquetas">
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                </div>
                                <div class="descripcion">
                                    <P>Lorem ipsum dolor sit amet consectetur adipisicing elit. Repudiandae neque eius incidunt assumenda nesciunt eum explicabo laudantium, soluta quaerat libero.</P>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="galeria" style="display: block;">
                        <div class="contenedor">
                            <div class="imagen">
                                <img src="https://iicca.edu.bo/wp-content/uploads/2019/09/comex0.jpg">
                            </div>
                            <div class="contenido">
                                <div class="titulo">
                                    <a href="https://iicca.edu.bo/galeria-prueba/">
                                        <h3>Lorem, ipsum dolor.</h3>
                                    </a>
                                </div>
                                <div class="etiquetas">
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                    <a href="https://iicca.edu.bo/galerias-lista-prueba/">
                                        <span>tag1</span>
                                    </a>
                                </div>
                                <div class="descripcion">
                                    <P>Lorem ipsum dolor sit amet consectetur adipisicing elit. Repudiandae neque eius incidunt assumenda nesciunt eum explicabo laudantium, soluta quaerat libero.</P>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="galeria" style="display: block;">
                        <div class="contenedor">
                            <div class="imagen">
                                <img src="https://iicca.edu.bo/wp-content/uploads/2017/10/N2.jpg">
                            </div>
                            <div class="contenido">
                                <div class="titulo">
                                    <a href="https://iicca.edu.bo/galeria-prueba/">
                                        <h3>Lorem, ipsum dolor.</h3>
                                    </a>
                                </div>
                                <div class="descripcion">
                                    <P>Lorem ipsum dolor sit amet consectetur adipisicing elit. Repudiandae neque eius incidunt assumenda nesciunt eum explicabo laudantium, soluta quaerat libero.</P>
                                </div>
                            </div>
                        </div>
                    </div>
                <!-- </div> -->
            <!-- </div> -->
        </div>
    </div>

</div> 